...
### Fixed
- **Selecting text in the Agent terminal while the agent has mouse tracking on (most full-screen TUIs) could lose the selection the instant you released the mouse button**, making copy/paste of a plain drag-selection unreliable. The workaround that lets a plain drag select at all — forcing xterm's own selection mode by re-dispatching the click with a modifier key held — picked Option-vs-Shift by sniffing `navigator.userAgent` for "Mac", but xterm's *own* check for the same thing (`shouldForceSelection`) uses the unrelated `navigator.platform` API instead. The two can disagree (platform detection is what's actively being deprecated across browsers), and when they do, den dispatches the wrong modifier — xterm's check silently fails, the drag never starts a real selection, and whatever highlight you saw was the TUI's own doing, gone the moment it decided to clear it. Den's check now matches xterm's exactly, so the two can never disagree.
