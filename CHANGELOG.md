...
- **Cloud sandboxes start showing up in the list.** `sbx --cloud ls` is now merged alongside the local listing — silently, so an account with no Docker Agentic Platform plan (or a runtime where the flag placement turns out wrong) just sees its local sandboxes as before. A cloud row carries a small cloud glyph next to its name, and once any cloud sandbox exists, a Location filter (All/Local/Cloud) appears next to Status in the sidebar's filter popover. Stop, remove, exec, and the Shell terminal tab now dispatch `--cloud` correctly for a cloud row — den remembers which listing reported each sandbox (same pattern as the existing per-sandbox agent map) so nothing extra had to be threaded through the UI. Deliberately not yet covered: publishing/unpublishing a cloud sandbox's ports (a cloud port spec is shaped differently — a bare port number, since the control plane assigns the public URL — not the host:container mapping the Ports panel builds), the Agent terminal tab's reattach (cloud's equivalent is `sbx --cloud attach`, a genuinely different command from `sbx run --name`, not the same verb with a flag added), `sbx move`, and cloud-scoped secrets/policy.
