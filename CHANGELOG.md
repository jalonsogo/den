...
### Added
- **sbx v0.42 support, without moving the floor.** Same treatment as v0.39: den's managed runtime pin moves to 0.42.0, but the floor stays at v0.38 — everything new is gated on the installed version (`hasCloud`, `hasKitArgs`, `hasNoWorkspaceCreate`, re-probed every 15s) rather than required, so an older runtime keeps working and simply isn't offered what it can't do. v0.42's one deprecation (`sbx run <name> --kit <ref>`) doesn't touch den: `sbx create` already invokes kits via `--kit <dir>` plus a resolved agent-name positional, the recommended shape, and den's only `sbx run` usage is reattaching to an already-created sandbox. Every flag den will still need to confirm against a real v0.42 binary (`--kit-arg`'s spelling, where `--cloud` sits in argv, `sbx move`'s shape) is tracked in [`docs/sbx-quirks.md`](docs/sbx-quirks.md), same as v0.39's were.
- **Sandboxes without a workspace** — New Sandbox gains a "No workspace bind mount" toggle (sbx 0.42+ only) for a sandbox with no folder mounted at all. It requires an explicit name, since there's no folder left to derive one from, and hides Isolation, which only means something relative to a mounted folder.
- **Kit arguments** — kits can now declare an `args:` block (name, description, default, required), authored in the kit editor's new Arguments capability alongside env vars and credentials. When a sandbox kit or mixin with declared args is selected in New Sandbox, its inputs appear right there and are passed as `--kit-arg name=value` — or `kit.name=value` when more than one selected kit declares the same arg name, so the flag stays unambiguous. sbx 0.42+ only; an older runtime still parses and round-trips a kit's `args:` block, it just doesn't show the input fields.
