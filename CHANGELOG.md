...
- **An MCP Servers page in the Library** — den's UI for the sbx v0.38 MCP gateway. Register a server once on the host and every sandbox reaches it through the gateway; the curated catalog registers in one click, and remote (URL) or local (command + args) servers can be entered by hand. Each row shows where it points and its authorization state, with **Authorize** streaming `sbx mcp auth` live, since that opens a browser on the host and may print a code. That host-side OAuth is the point: authorizing from *inside* a sandbox means a browser callback the host can't reach, which is the wall the kit-based MCP flow hits. Kits can still register MCPs the old way — that path is per-sandbox and unchanged.
