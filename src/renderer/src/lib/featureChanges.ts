...
export async function bringSandboxToHost(sandbox: Sandbox, deleteAfter: boolean): Promise<void> {
  ...
  const res = await window.den?.sandboxFetchWork(sandbox.name, ws).catch(() => null)
  ...
  if (res.hasRemote) {
    if (confirm(`Fetched changes to "${branch}".\n\nOpen a pull request? (Cancel keeps the branch for review.)`)) {
      const pr = await window.den?.sandboxOpenPr(ws, branch).catch(() => null)
      if (pr?.ok && pr.url) { window.den?.openPath(pr.url); done = true }
    ...
    const m = await window.den?.sandboxMergeBranch(ws, branch).catch(() => null)
  ...
  if (done && deleteAfter && confirm(`Delete the sandbox "${sandbox.name}" now?`)) {
    ...
    window.den?.deleteSandbox(sandbox.name).catch(() => {})
  }
}
