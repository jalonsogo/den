...
function DockerIcon({ size = 14 }: { size?: number }) {
  return (
...
export function AgentIcon({ agent, size = 14, style }: AgentIconProps) {
  const p = { size, style }
  switch (agent) {
    case 'claude':         return <ClaudeColor  {...p} />
    case 'claude-bedrock': return <BedrockColor {...p} />
    case 'codex':          return <CodexColor   {...p} />
    case 'gemini':         return <GeminiColor  {...p} />
    case 'opencode':       return <OpenCodeMono {...p} />
    case 'copilot':        return <CopilotMono  {...p} />
    case 'cursor':         return <CursorMono   {...p} />
    case 'docker-agent':   return <DockerIcon   size={size} />
