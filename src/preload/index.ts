...
const api = {
  ...
  sshFeatureStatus: ()                  => ipcRenderer.invoke('minipit:ssh-feature-status'),
  setSshFeature: (enabled: boolean, restart?: boolean) => ipcRenderer.invoke('minipit:set-ssh-feature', enabled, restart),
  openVscode:    (name: string)         => ipcRenderer.invoke('minipit:open-vscode', name),
  codeAvailable: ()                     => ipcRenderer.invoke('minipit:code-available'),
